...
# CLASSIFIER_TYPE: classifier type to use can be SVM or gradient_boosting
...
classifier=${CLASSIFIER_TYPE:-gradient_boosting}
...
for n_estimator in ${n_estimators[@]}; do
    for top_k in ${top_ks[@]}; do
        for ngram_size in ${ngram_sizes[@]}; do
            ...
            echo "--n-estimators $n_estimator -k $top_k --ngram-size $ngram_size --datafile $data_path --exp-name $exp_name --classifier-type $classifier" >> $args_file
            # python3 $script_path \
            #     --n-estimators $1 \
            #     --top-k-ngrams $top_k \
...
xargs -a $args_file -n 12 -P ${1:-2} python3 $script_path
